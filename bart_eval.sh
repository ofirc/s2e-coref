#!/bin/bash
date > bart_base_eval_without_coref_start.txt
python run_coref.py \
  --output_dir=output \
  --cache_dir=cache \
  --model_type=facebook/bart-base \
  --model_name_or_path=../outputs/model_files_checkpoint_epoch3 \
  --tokenizer_name=../outputs/model_files_checkpoint_epoch3 \
  --config_name=../outputs/model_files_checkpoint_epoch3/config.json \
...
date > bart_base_eval_without_coref_end.txt
